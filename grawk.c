/*
 * Grawk! The O.G. Micro "Grep + Awk"!
 *
 * The original 'grawk' written in 2008 as a high-
 * performance tiny filtering tool to help admins
 * recover spiraling web servers by sawing through 
 * logs and processes information output to reveal 
 * causes quickly. 
 * 
 * Now being transplanted into a more modern library, 
 * with this version preserved for posterity and utility.
 *
...
#define MAINTAINER "Tim Post <tinkertim@gmail.com>"
...
static char *grawk_readline(FILE *);
...
/* grawk_readline() - read a line from the file handle.
 * Return an allocated string */
static char *grawk_readline(FILE *fp)
{
	char *str = (char *)NULL;
...
static int awkcomp(awk_pat_t *awk, char *apattern)
{
	...
	char ch;
	for (i = 0; i < (int)strlen( apattern ); i++) {
		ch = apattern[i];
		if (inString && ch != '"' && ch != '\\') {
			wrkbuf[offs++] = ch;
			continue;
